...
export function makeRelative(pathSelf, pathTarget) {
    ...
    while (
        pathSelfArray.length > 0 &&
        pathTargetArray.length > 0 &&
        pathSelfArray[0] === pathTargetArray[0]
    ) {
        pathSelfArray.shift();
        pathTargetArray.shift();
    }
    return (
        ('../'.repeat(pathSelfArray.length) || './') +
        pathTargetArray.map(x => x + '/').join('') +
        targetBasename
    );
}
    ...
    if (typeof item === 'string') {
        ...
        return {
            ...
            children: children.map(child =>
                resolveItem(child, pages, base, true)
            ),
